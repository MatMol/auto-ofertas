/**
 * Seed script: scrapes real listings from MercadoLibre search results HTML,
 * normalizes them, calculates deal scores, and writes to a local JSON file.
 *
 * Usage: pnpm seed
 *
 * Fetches multiple pages from autos.mercadolibre.com.ar (SSR HTML with
 * embedded polycard JSON), extracts listing data, and writes seed-data.json.
 */
        ...
        if (jsonStr) {
          try {
            ...
            const data = JSON.parse(raw) as {
              metadata?: PolycardData["metadata"];
              pictures?: PolycardData["pictures"];
              components?: PolycardComponent[];
            };
...
function fixProvince(locationText: string): string {
  // Location text is like "Pilar - Bs.As. G.B.A. Norte"
...
function parseTitle(title: string): { brand: string; model: string; version: string | null } {
  ...
  const model = rest[0] ?? "";
  const version = rest.length > 1 ? rest.slice(1).join(" ") : null;
...
function inferAttributes(listing: Listing): void {
  ...
  if (!listing.transmission) {
    if (/\bat\b|automát|automatic|cvt|dsg|tiptronic|powershift|dct/.test(title)) {
      listing.transmission = "automatica";
    } else if (/\bmt\b|manual/.test(title)) {
  ...
  if (!listing.fuelType) {
    ...
    } else if (/gnc/.test(title)) {
      listing.fuelType = "gnc";
  ...
  if (!listing.bodyType) {
    const brandModel = `${listing.brand} ${listing.model}`.toLowerCase();
    ...
    if (pickups.some((p) => brandModel.includes(p))) listing.bodyType = "pickup";
    else if (suvs.some((s) => brandModel.includes(s))) listing.bodyType = "suv";
    else if (sedans.some((s) => brandModel.includes(s))) listing.bodyType = "sedan";
    else if (hatchbacks.some((h) => brandModel.includes(h))) listing.bodyType = "hatchback";
  }
}
...
function calculateScores(listings: Listing[]): void {
  ...
  for (const group of groups.values()) {
    ...
    // Always use priceArs for comparisons so USD and ARS listings are comparable
    ...
    for (const l of group) {
      ...
      const trusted =
        l.verificationBadge === "ml_verified" ? 80 :
        l.verificationBadge === "concesionaria" ? 65 :
        l.sellerType === "concesionaria" ? 55 : 35;
...
function isSuspiciousTitle(listing: Listing): boolean {
  const fullText = `${listing.model} ${listing.version ?? ""} ${listing.sourceUrl}`.toLowerCase();
  return ANTICIPO_KEYWORDS.some((kw) => fullText.includes(kw));
}

function filterSuspiciousListings(listings: Listing[]): Listing[] {
  // Build median prices per brand+model (only from listings with reasonable prices)
  ...
  return listings.filter((l) => {
    ...
    // Strategy 1: keyword match in title/URL + price suspiciously low
    if (isSuspiciousTitle(l)) {
      if (!median || l.priceArs < median * 0.5) {
        return false;
      }
    }

    // Strategy 2: pure price outlier — price < 30% of median for that model
    // Only for groups with enough data to be confident
    if (median && groups.get(key)!.length >= 5) {
      const ratio = l.priceArs / median;
      if (ratio < 0.30) {
        return false;
      }
    }

    // Strategy 3: 0km listing priced absurdly low (< 40% of median)
    // These are almost always anticipos
    if (l.isNew && median) {
      const ratio = l.priceArs / median;
      if (ratio < 0.40) {
        return false;
...
async function main() {
  console.log("Scraping MercadoLibre search results...\n");
  ...
  for (let page = 0; page < PAGES_TO_FETCH; page++) {
    ...
    try {
      ...
      console.log(` ${added} new listings (${allListings.length} total)`);

      if (added === 0 && polycards.length === 0) {
        console.log("  No polycards found — may have hit the end");
        break;
      }
    } catch (err) {
  ...
  if (allListings.length === 0) {
    console.log("\nNo listings scraped. Try again later.");
    process.exit(1);
  }

  console.log(`\nInferring attributes from titles...`);
  for (const l of allListings) inferAttributes(l);

  console.log(`Filtering out anticipo/cuota listings...`);
  ...
  console.log(`  Removed ${removed} suspicious listings (${filtered.length} remaining)`);
  ...
  for (const l of filtered) brands.set(l.brand, (brands.get(l.brand) ?? 0) + 1);

  const withFuel = filtered.filter((l) => l.fuelType).length;
  const withTrans = filtered.filter((l) => l.transmission).length;

  const scores = { great: 0, good: 0, fair: 0, low: 0 };
  for (const l of filtered) {
    if (l.dealScore >= 80) scores.great++;
    else if (l.dealScore >= 65) scores.good++;
    else if (l.dealScore >= 50) scores.fair++;
    else scores.low++;
  }

  const outPath = resolve(__dirname, "../src/lib/db/seed-data.json");
  writeFileSync(outPath, JSON.stringify(filtered, null, 2));

  console.log(`\nWrote ${filtered.length} listings to src/lib/db/seed-data.json (${removed} filtered out)`);
  console.log(`\nStats:`);
  console.log(`  Brands: ${brands.size} (top: ${[...brands.entries()].sort((a, b) => b[1] - a[1]).slice(0, 8).map(([b, c]) => `${b}: ${c}`).join(", ")})`);
  console.log(`  With fuel type: ${withFuel}, transmission: ${withTrans}`);
  console.log(`  Scores: ${scores.great} great (80+), ${scores.good} good (65-79), ${scores.fair} fair (50-64), ${scores.low} low (<50)`);
  console.log(`\nRestart the dev server to see real data.`);
}
